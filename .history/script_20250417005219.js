...
document.addEventListener('alpine:init', () => {
    Alpine.data('settings', () => ({
        activeTab: 'notifications', // Default active tab
        passwordVisible: false, // Toggle for password visibility
        avatarPreview: '', // Preview for avatar upload
        settings: {
            notifications: {
                email: true,
                sms: false,
        ...
        saveSettings() {
            // Validate form data
            if (!this.settings.account.username || !this.settings.account.email || !this.settings.account.currentPassword) {
                alert("Please fill in all required fields.");
                return;
            }
        ...
        validateUsername() {
            this.errors.username = this.settings.account.username.length < 3 ? "Username must be at least 3 characters long." : "";
        },

        // Method to validate email
        validateEmail() {
            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            this.errors.email = !emailPattern.test(this.settings.account.email) ? "Please enter a valid email address." : "";
        },

        // Method to validate password match
        validatePasswordMatch() {
            this.errors.confirmPassword = this.settings.account.newPassword !== this.settings.account.confirmPassword ? "Passwords do not match." : "";
        },

        // Method to check password strength
        checkPasswordStrength() {
            const password = this.settings.account.newPassword;
            ...
            if (this.passwordCriteria.length) this.passwordStrengthPercent += 25;
            if (this.passwordCriteria.uppercase) this.passwordStrengthPercent += 25;
            if (this.passwordCriteria.number) this.passwordStrengthPercent += 25;
            if (this.passwordCriteria.special) this.passwordStrengthPercent += 25;
