...
document.querySelectorAll('.generate-report-btn').forEach(button => {
  ...
  function generateMockReport(reportName) {
   ...
   document.addEventListener('DOMContentLoaded', function() {
  // Initialize Alpine.js app if using Alpine
  if (window.Alpine) {
    window.Alpine.data('settingsApp', settingsApp);
  }
});
...
function settingsApp() {
  return {
    ...
    toggleDarkMode() {
      ...
      // Actually apply dark mode to the body element
      if (this.darkMode) {
    ...
    validateUsername() {
      if (this.settings.account.username.length < 3) {
    ...
    validateEmail() {
      ...
      if (!emailRegex.test(this.settings.account.email)) {
    ...
    validatePasswordMatch() {
      if (this.settings.account.newPassword && 
          this.settings.account.newPassword !== this.settings.account.confirmPassword) {
    ...
    checkPasswordStrength() {
      ...
      // Calculate strength
      let strength = 0;
      if (this.passwordCriteria.length) strength += 25;
      if (this.passwordCriteria.uppercase) strength += 25;
      if (this.passwordCriteria.number) strength += 25;
      if (this.passwordCriteria.special) strength += 25;
    ...
    saveSettings() {
      ...
      if (this.errors.username || this.errors.email || this.errors.confirmPassword) {
        alert('Please fix the errors before saving.');
        return;
      }
      
      // Here you would usually send the data to your server via fetch or axios
    ...
    init() {
      ...
      if (window.matchMedia('(prefers-color-scheme: dark)').matches && localStorage.getItem('darkMode') === null) {
        this.darkMode = true;
