...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $connection->query("UPDATE vehicles SET vehicle_name='$name', vehicle_number='$number', vehicle_type='$type', status='$status' WHERE id=$id");
...
<body class="p-8 bg-gray-100">
  <div class="max-w-md mx-auto bg-white p-6 rounded-xl shadow-md">
    ...
    <form method="POST">
      <input class="w-full p-2 mb-2 border rounded" name="vehicle_name" value="<?= $vehicle['vehicle_name'] ?>" required />
      <input class="w-full p-2 mb-2 border rounded" name="vehicle_number" value="<?= $vehicle['vehicle_number'] ?>" required />
    ...
    <a href="vehicle_list.php" class="block mt-4 text-blue-600 hover:underline">← Cancel</a>
  </div>
</body>
</html>
