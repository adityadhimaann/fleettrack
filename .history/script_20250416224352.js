...
const map = L.map('map').setView([40.7128, -74.0060], 13);
        ...
        const vehicles = {
            "truck1": {
                ...
                fuelLevel: 78,
                engineOn: true,
                distanceTraveled: 124.5,
                location: "Manhattan, New York"
            },
            "car1": {
                name: "Company Car 1",
                id: "CC-001",
                initialPosition: [40.7328, -73.9860],
                driver: "Emily Johnson",
                speed: 32,
                fuelLevel: 62,
                engineOn: true,
                distanceTraveled: 86.2,
                location: "Brooklyn, New York"
            },
            "van1": {
                name: "Service Van 1",
                id: "SV-001",
                initialPosition: [40.6928, -74.0260],
                driver: "Mike Davis",
                speed: 0,
                fuelLevel: 45,
                engineOn: false,
                distanceTraveled: 213.7,
                location: "Jersey City, New Jersey"
            }
        };

        // Search for vehicle function
        ...
        function searchVehicle() {
            const vehicleName = document.getElementById('vehicleName').value.toLowerCase();
            ...
            if (vehicles[vehicleName]) {
                ...
                document.getElementById('vehicleTitle').textContent = `${vehicle.name} (ID: ${vehicle.id})`;
                document.getElementById('lastUpdate').querySelector('span').textContent = new Date().toLocaleString();
                document.getElementById('currentSpeed').textContent = `${vehicle.speed} km/h`;
                document.getElementById('distanceTraveled').textContent = `${vehicle.distanceTraveled.toFixed(1)} km`;
                document.getElementById('fuelLevel').textContent = `${vehicle.fuelLevel}%`;
                document.getElementById('engineStatus').textContent = vehicle.engineOn ? 'Running' : 'Off';
                ...
                const vehicleIcon = L.divIcon({
                    html: '<i class="fas fa-truck" style="font-size: 24px; color: #3b82f6;"></i>',
                    className: 'vehicle-marker',
                    iconSize: [30, 30],
                    iconAnchor: [15, 15]
                });
                
                vehicleMarker = L.marker(vehicle.initialPosition, { icon: vehicleIcon }).addTo(map);
                map.setView(vehicle.initialPosition, 14);
                
                // Simulate vehicle movement
                startVehicleSimulation(vehicle);
            } else {
        ...
        function startVehicleSimulation(vehicle) {
            // Clear previous path
            pathCoordinates = [vehicle.initialPosition];
            
            // Generate random path around initial position
            const simulateMovement = setInterval(() => {
                if (vehicle.engineOn && Math.random() > 0.2) {
                    // Calculate next position (random movement)
                    ...
                    const newPos = [lastPos[0] + latChange, lastPos[1] + lngChange];
                    ...
                    vehiclePath = L.polyline(pathCoordinates, {color: '#3b82f6', weight: 3}).addTo(map);
                    
                    // Update vehicle info
                    vehicle.speed = Math.floor(Math.random() * 20) + 30;
                    vehicle.distanceTraveled += vehicle.speed * 0.001; // Approximate distance calculation
                    vehicle.fuelLevel = Math.max(0, vehicle.fuelLevel - 0.1);
                    
                    document.getElementById('currentSpeed').textContent = `${vehicle.speed} km/h`;
                    document.getElementById('distanceTraveled').textContent = `${vehicle.distanceTraveled.toFixed(1)} km`;
                    document.getElementById('fuelLevel').textContent = `${Math.round(vehicle.fuelLevel)}%`;
                    document.getElementById('lastUpdate').querySelector('span').textContent = new Date().toLocaleString();
                }
            }, 2000);
            
            // Clean up interval after 5 minutes to prevent memory issues
            setTimeout(() => {
        ...
        window.addEventListener('load', function() {
            document.getElementById('vehicleName').value = 'truck1';
            searchVehicle();
        });